package com.oechslerbernardo.mongodbteste.presentation.write.components
...
import com.oechslerbernardo.mongodbteste.presentation.write.WriteState
...
fun WriteTopBar(
    ...
    CenterAlignedTopAppBar(
        navigationIcon = {
            IconButton(onClick = { onEvent(WriteEvent.OnBackClick) }) {
                Icon(
        ...
        title = {
            Text(text = if (state.selectedDiary != null) state.title else "New Diary")
        },
        actions = {
            if (dateTimeUpdated) {
                IconButton(onClick = {
...
fun DeleteDiaryAction(
    ...
    DropdownMenu(
        expanded = expanded,
        onDismissRequest = { state.isDialogOpen = false }
    ) {
        DropdownMenuItem(
            text = {
                Text(text = "Delete")
    ...
    DisplayAlertDialog(
        ...
        onDialogClosed = { onEvent(WriteEvent.OnDialogDismiss) },
        onYesClicked = { onEvent(WriteEvent.OnDeleteClick) }
    )
